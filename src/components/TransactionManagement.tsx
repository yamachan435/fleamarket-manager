...
export default function TransactionManagement() {
  ...
  const handleStartTransaction = async (productId: string) => {
    if (!confirm('この商品を取引中にしますか？')) return
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-4">取引</h2>

      {loading ? (
        <p className="text-center text-gray-500 py-8">読み込み中...</p>
      ) : products.length === 0 ? (
        <p className="text-center text-gray-500 py-8">取引可能な商品がありません</p>
      ) : (
      ...
      {selectedProductId && (
        <div className="mt-6 flex justify-end">
          <button
            onClick={() => handleStartTransaction(selectedProductId)}
